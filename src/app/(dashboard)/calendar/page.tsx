...
export default function CalendarPage() {
  ...
  const handleAddEvent = () => {
    ...
    toast.success(`Event "${newTitle}" added to calendar`);
  };
  ...
  const upcomingEvents = events
    .filter(e => e.date >= '2026-03-12' && e.date <= '2026-03-19')
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-5">
                    ...
                    >
                      ...
                      <div className="space-y-0.5">
                        {dayEvents.slice(0, 2).map(ev => (
                          <div
                            key={ev.id}
                            className={`w-full h-1.5 rounded-full ${categoryConfig[ev.category].dot}`}
                          />
                        ))}
                        {dayEvents.length > 2 && (
              ...
              <CardContent className="pt-0">
                {selectedEvents.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-4">
                    ...
                    {isPrincipal && ' Click "Add Event" to create one.'}
                  </p>
                ) : (
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="pt-0 space-y-2">
              {Object.entries(categoryConfig).map(([key, cfg]) => {
                const count = events.filter(e => e.category === key as EventCategory && e.date.startsWith(`${year}-${String(month + 1).padStart(2, '0')}`)).length;
                return (
      ...
      {isPrincipal && (
        <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Add Event — {selectedDate?.split('-').reverse().join('/')}</DialogTitle>
              <DialogDescription>Add a new event to the academic calendar</DialogDescription>
            </DialogHeader>
            <div className="space-y-3">
              <div>
                <label className="text-xs font-medium text-muted-foreground block mb-1.5">Event Title *</label>
                <Input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="e.g. Unit Test 3" />
              </div>
              <div>
              ...
              <div>
                ...
                <Input value={newDesc} onChange={e => setNewDesc(e.target.value)} placeholder="Details about this event" />
              </div>
              <div className="flex gap-3 pt-1">
                <Button variant="outline" className="flex-1" onClick={() => setShowAddModal(false)}>Cancel</Button>
                <Button variant="primary" className="flex-1" onClick={handleAddEvent} disabled={!newTitle.trim()}>Add Event</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
